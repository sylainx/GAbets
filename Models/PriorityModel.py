...
class PrioritiesModel:
    ...
    def save(self):
        try:
            ...
            requete = "INSERT INTO `priority`(`id`, `title`, `ratio`, `visible`)\
                VALUES (%s,%s,%s,%s) "
    ...
    def search(self, id):
        try:
            ...
            requete = " SELECT * FROM `priority` WHERE id=%s "
            self.cursor = self.conn.cursor()
            valeur = (id,)
    ...
    def update(self, id):
        try:
            obj = DBConnection()
            self.conn = obj.connection()
            requete = " UPDATE `priority` SET title=%s, ratio=%s, visible=%s,\
                WHERE id=%s"
    ...
    def delete(self, id):
        ...
        try:
            obj = DBConnection()
            conn = obj.connection()
            requete = " DELETE FROM `priority` WHERE id=%s "
            valeur = (id,)
            self.cursor = self.conn.cursor()
