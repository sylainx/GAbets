...
class UsersModel:
    ...
    def save(self):
        try:
            ...
            self.cursor.execute(
                "SELECT * FROM `users` WHERE id = %s", (inserted_id,))
            ...
            if inserted_row:
                ...
                QMessageBox.information(
                    None, "Confirmation - usr", "Usr- Enregistrement reussi", QMessageBox.Ok)
                return inserted_id

            else:
                QMessageBox.warning(
                    None, "Error - usr", "Usr- Quelque chose s'est mal passé", QMessageBox.Ok)

        except mysql.connector.Error as erreur:
            QMessageBox.warning(None, "Erreur utilisateur", "Usr- Erreur " +
    ...
    def show(self):
        try:
            ...
            QMessageBox.warning(
                None, "Erreur utilisateur", "Usr- Impossible d'afficher les informations " + str(erreur), QMessageBox.Ok)
            # fermer le cursor
            self.cursor.close()
            # tester si la connexion est ouverte
            if self.conn.is_connected():
    ...
    def search(self, code):
        try:
            ...
            print(erreur)
            QMessageBox.warning(
                None, "Erreur utilisateur", "Usr- Impossible de rechercher un utilisateur la BD " + str(erreur), QMessageBox.Ok)
            # fermer le cursor
            self.cursor.close()
            # tester si la connexion est ouverte
            if self.conn.is_connected():
    ...
    def searchCodeById(self, usr_id):
        try:
            obj = DBConnection()
            self.conn = obj.connection()
            requete = " SELECT `code_user` FROM `USERS` WHERE id=%s "
            self.cursor = self.conn.cursor()
            valeur = (usr_id,)
            self.cursor.execute(requete, valeur)
            self.liste = self.cursor.fetchone()
            ...
            QMessageBox.warning(
                None, "Erreur utilisateur", "Usr- Impossible de rechercher un utilisateur par id a la BD " + str(erreur), QMessageBox.Ok)
            # fermer le cursor
            self.cursor.close()
            # tester si la connexion est ouverte
            if self.conn.is_connected():
    ...
    def searchUserByCode(self, code):
        try:
            obj = DBConnection()
            self.conn = obj.connection()
            requete = " SELECT * FROM `USERS` WHERE code_user=%s "
            self.cursor = self.conn.cursor()
            valeur = (code,)
            self.cursor.execute(requete, valeur)
            self.liste = self.cursor.fetchone()
            ...
            QMessageBox.warning(
                None, "Erreur utilisateur", "Usr- Impossible de rechercher l'utilisateur par code à la BD " + str(erreur), QMessageBox.Ok)
            # fermer le cursor
            self.cursor.close()
            # tester si la connexion est ouverte
            if self.conn.is_connected():
    ...
    def update(self):
        try:
            ...
            self.conn.commit()
            QMessageBox.information(
                None, "Confirmation", "Usr- Modification reussie", QMessageBox.Ok)
        except mysql.connector.Error as erreur:
            QMessageBox.warning(
                None, "Erreur utilisateur", "Usr- Impossible d'acceder a la BD " + str(erreur), QMessageBox.Ok)
            # fermer le cursor
            self.cursor.close()
            # tester si la connexion est ouverte
            if self.conn.is_connected():
    ...
    def delete(self, code):
        ...
        try:
            ...
            rep = QMessageBox.question(
                None, "Confirmation", "Usr- Voulez-vous supprimer cette inscription", QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            if rep == QMessageBox.Yes:
                self.cursor.execute(requete, valeur)
                self.conn.commit()
        except mysql.connector.Error as erreur:
            QMessageBox.warning(
                None, "Erreur utilisateur", "Usr- Impossible de surpprimer cett inscription: " + str(erreur), QMessageBox.Ok)
            # fermer le cursor
            self.cursor.close()
            # tester si la connexion est ouverte
            if self.conn.is_connected():
